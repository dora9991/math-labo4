...
export default function Shop({ player, onBuy, onDiscard, onHeal, onBack }) {
  const coins = player.coins ?? 0;
  const lv = levelFromXp(player.xp);
  const maxHp = getPlayerBattleStats(lv).maxHp;
              ...
              {byKind[kind].map((it) => {
                ...
                return (
                  <div key={it.id} style={{
                    ...
                    <div style={{ fontSize: 10.5, color: "rgba(255,255,255,.6)", lineHeight: 1.35, minHeight: 42 }}>{it.desc}</div>
                    {locked ? (
